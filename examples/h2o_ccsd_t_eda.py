...
print(res.summary())
print('\nE_(T)^A  (U^{0,0}) :', res.e_t_occ.round(7))
print('E_(T)^A  (U^{2,2}) :', res.e_t_vir.round(7))
print('E_T[4]^A           :', res.e_t4.round(7))
print('E_ST[5]^A          :', res.e_st5.round(7))
print('E_(T) total        : %.10f' % res.e_t.sum())

res_nao = eda_ccsd_t.EDA(mycc, orbital_basis='nao').kernel()
print('\nNAO-EDA: E_(T)^A =', res_nao.e_t.round(7), ' E_CCSD(T)^A =', res_nao.e_tot.round(6))
...
print(f"{'Atom':<6}{'E_CCSD(T)^A':>16}{'E(atom)':>16}{'Difference':>14}")
...
print(f"{'Sum':<6}{res.e_tot.sum():>16.5f}{e_atoms_sum:>16.5f}"
      f"{e_tot - e_atoms_sum:>14.5f}   (= -atomization energy)")
