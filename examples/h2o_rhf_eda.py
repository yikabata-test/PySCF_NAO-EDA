...
print(f"{'Atom':<6}{'E_TOT^A':>16}{'E(atom, UHF)':>16}{'Difference':>14}")
for ia in range(mol.natm):
    symb = mol.atom_symbol(ia)
    d = res.e_tot[ia] - e_atom[symb]
    print(f"{symb + str(ia):<6}{res.e_tot[ia]:>16.5f}{e_atom[symb]:>16.5f}{d:>14.5f}")
e_atoms_sum = sum(e_atom[mol.atom_symbol(ia)] for ia in range(mol.natm))
print(f"{'Sum':<6}{res.e_tot.sum():>16.5f}{e_atoms_sum:>16.5f}"
      f"{mf.e_tot - e_atoms_sum:>14.5f}   (= -atomization energy)")
